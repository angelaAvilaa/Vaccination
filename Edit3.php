...
$editID = $editCN = $editCA  = $editV = ''; // Initialize variables

// Fetch the data for the edit form
if (isset($_POST['edit'])) {
    $editID = $_POST['editID'];
    $result = mysqli_query($connection, "SELECT * FROM san_juan WHERE ID = $editID");
    ...
    if ($result && mysqli_num_rows($result)) {
        ...
        echo '<script>alert("User not found!"); window.location.href="/VACCINATION/San Juan.php";</script>';
        exit();
    }
}
...
if (isset($_POST['update'])) {
    ...
    $query = "UPDATE san_juan SET ChildName = ?, ChildAge = ?, Vaccine = ? WHERE ID = ?";
    $stmt = mysqli_prepare($connection, $query);
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, 'sssi', $updateCN, $updateCA,$updateV, $updateID);
        if (mysqli_stmt_execute($stmt)) {
            echo '<script>alert("Successfully Edited!"); window.location.href="/VACCINATION/San Juan.php";</script>';
        } else {
